...
const Home: NextPage = () => {
  ...
  const [finishedBidding, setFinishedBidding] = useState(false);
          ...
          {currentRound && (
            <>
              ...
              <RoundBids
                ...
                setFinishedBidding={setFinishedBidding}
              />
            </>
          )}
        </>
...
function BidPage({ roundId }: { roundId: string }) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleOnSubmit} className="mt-8 flex gap-1 text-white">
        <label>
          <input
...
function RoundBids({ roundId, setFinishedBidding }: { roundId: string }) {
  console.log("RoundBids: roundId", roundId);
  const { isLoading, data: bids } = useQuery<Bid[]>({
    queryKey: "roundBids",
    queryFn: () =>
  ...
  const finishedBidding = bids?.some(
    (bid) => typeof bid.amount !== "undefined"
  );
  if (finishedBidding) {
    console.log('fnished bidding')
    setFinishedBidding(true);
  }
  console.log("finishedBidding", finishedBidding);
